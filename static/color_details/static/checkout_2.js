...
{	
	...
	item=strasse+" "+nummer+","+apt+","+stadt+","+plz+","+unternehmensdetails+","+vorname+","+nachname+","+telefonnummer+",lieferdetails,"+zahlungsoption+","+name_karteninhaber+","+kreditkarten_nummer__+","+ablaufdatum+","+pruefnummer+","+preis_+","+"2.95"+","+rabatt_+","+document.getElementById("gutschein_input").value.toUpperCase; 
			$.ajax({
			type: "POST",
		...
		window.top.location.href = "/polls/hello/order_summary/"; 
...
function vorbestellung_abrufen(datum_,uhrzeit_)
{
			...
			box=box+"<br><br><br>"
			 ...
			 while(i<=cart.length-1)
			 {
				...
				{	
					...
					box=box+"<div style='float:left; font-size:12px;width:575px;'><div style='float:left;'><div style='cursor:pointer' onclick='gerichte_detail("+i+")'><img src='"+gericht_details_ermitteln(cart[i][0],5)+"' style='width:45px;height:45px;float:left;'/><div style='margin-left:10px;float:left;'>"+cart[i][0]+"<br><div style='float:left;font-size:12px;'>"+replace_dot_comma(parseFloat(gericht_details_ermitteln(cart[i][0],6)))+" EUR</div> </div></div></div>";
 ...
 function gerichte_detail(link_)
 {
	 ...
	 window.top.location.href = "/polls/hello/"+cart[link_][3]+"/"+cart[link_][0]+"/";; 	 
