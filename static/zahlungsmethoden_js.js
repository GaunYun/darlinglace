...
function zahlungsmethode_speichern(index)
{
		...
		index_hilf_credit_card=index;
		index_hilf_credit_card_=index_hilf_credit_card


	if (index_hilf_credit_card_==-1)
		index_2=0;
	else
		index_2=index_hilf_credit_card_+1;
	...
	if(check_field_empty(index_2)=="ok")
	{
			...
			{	
				var hinzufuegen="0";
				index_2=index_hilf_credit_card_+1
				indexnummer=zahlungsmethoden[index_hilf_credit_card_].indexnummer
				standard="nein"
			}
		...
		document.getElementsByClassName("card-number")[0].value=document.getElementsByClassName("eingabefelder_kartennummer")[index_2].value.replace(/-/g , "")

		 document.getElementsByClassName("card-cvc")[0].value=document.getElementsByClassName("eingabefelder_pruefziffer")[index_2].value;

		 document.getElementsByClassName("card-expiry-month")[0].value=document.getElementsByClassName("eingabefelder_ablaufdatum_monat")[index_2].value.substring(0,2);

		 document.getElementsByClassName("card-expiry-year")[0].value=document.getElementsByClassName("eingabefelder_ablaufdatum_jahr")[index_2].value;	 	 

		 document.getElementsByClassName("card-holdername")[0].value=document.getElementsByClassName("eingabefelder_karteninhaber")[index_2].value;
 ...
 {
	if (index_hilf_credit_card==-1)
		index_2=0;
	else
		index_2=index_hilf_credit_card+1;
		
	kreditkarten_show("",index_hilf_credit_card)
	if(check_field_empty(index_2)=="ok")
	{
			...
			{	
				var hinzufuegen="0";
				index_2=index_hilf_credit_card+1
				indexnummer=zahlungsmethoden[index_hilf_credit_card].indexnummer
				standard="nein"
			}
		...
		{		
			...
			$.ajax({
				...
				dataType: "json",
				data: { "hinzufuegen":hinzufuegen,"indexnummer":indexnummer,"zahlungsoption": 0,"card_type":card_type,"token":token,'standard':standard},
...
function zahlungsmethode_loeschen(index)
{
	...
	$.ajax({
		...
		data: { "hinzufuegen":hinzufuegen,"indexnummer":zahlungsmethoden[index].indexnummer,"zahlungsoption": "0"},
		success: function(data) {
...
function zahlungsmethoden_laden(daten)
{
	...
	if (zahlungsmethoden!="")
	{
		while (i<=zahlungsmethoden.length-1)
		{
			...
			box=box+"<div class='bearbeiten_feld' onclick='zahlungsmethode_loeschen("+i+")'>Löschen</div>"
			box=box+"</div>"
			box=box+"</div>"
...
function check_field_empty(index)
{
 ...
 function kreditkarten_show(evt,index)
 {
	...
	if(key==48 || key==49 || key==50 || key==51 || key==52 || key==53 || key==54 || key==55 || key==56 || key==57 || key==8 || key==17 || key==91 || key==67 || key==86 || key=="" || key==9)	
	{
