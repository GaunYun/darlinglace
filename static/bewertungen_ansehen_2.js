...
/*	i=0;
	while (i<=bestelldetails.length-1)
	{
		document.getElementsByClassName("button_bewertungen")[i].id=i;
		i=i+1;
	}
	*/
	...
	while (i<=bestellungen.length-1)
	{
		...
		while (j<=bestelldetails.length-1)
		{
			...
			if(bestelldetails[j].bestellnummer==bestellungen[i].bestellnummer)
			{
				...
				document.getElementsByClassName("order_picture")[j].src=pictures_bestelldetails[0].link
		...
		while (j<=bestelldetails.length-1)
		{
			...
			if(bestelldetails[j].bestellnummer==bestellungen[i].bestellnummer)
			{
				...
				if(document.getElementsByClassName("headline_input")[j].value=="")
				{
					...
					alert_userdata("ÜBERSCHRIFT HINZUFÜGEN","Bitte gib deiner Bewertung noch eine Überschrift.")
					feedback="false";
					i=bestellungen.length;
					break;
				}
				...
				if(document.getElementsByClassName("textarea_bewertung")[j].value=="")
				{
					...
					alert_userdata("DETAILS ZUR BEWERTUNG HINZUFÜGEN","Bitte gib liefere noch Details zu deiner Bewertung.")
					feedback="false";
					i=bestellungen.length;
					break;
				}			
				...
				if(bewertung[j]=="")
				{
					...
					alert_userdata("BEWERTUNG HINZUFÜGEN","Bitte bewerte deine Bestellung noch.")
					feedback="false";
					i=bestellungen.length;
					break;
				}			
				...
				if(passform[j]=="")
				{
					...
					alert_userdata("PASSFORM HINZUFÜGEN","Bitte gib noch Details zur Passform an: wie gut hat der BH gepasst?")
					feedback="false";
					i=bestellungen.length;
				}	
			}		
...
function bestellungen_bearbeiten()
{
	...
	window.location.href="/account_page/bestellungen_bearbeiten/";
...
function bewertung_bearbeiten(index,index_1)
{
	...
	window.location.href="/account_page/bewertungen_bearbeiten/"+bestellungen[index].bestellnummer+"/"+bestelldetails[index_1].linkzugericht+"/";
}
		...
		while (i<=bestellungen.length-1)
		{
			...
			while (j<=bestelldetails.length-1)
			{
				...
				if(bestelldetails[j].bestellnummer==bestellungen[i].bestellnummer)
				{
					...
					$.ajax({
					type: "POST",
					url: "/account_page/bewertung_speichern/",
					dataType: "json",
					...
					success: function(data) {
						page_load(1);
							...
							zaehler_=zaehler_+1
								if(zaehler==zaehler_)
									alert_userdata_redirect_account_page("BEWERTUNG ERFOLGT","Vielen Dank für Deine Bewertung. Damit hilfst Du anderen Kunden den passenden BH zu finden.")
